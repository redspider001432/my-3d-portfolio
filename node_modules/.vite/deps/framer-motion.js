...
// node_modules/framer-motion/dist/es/animation/js/driver-frameloop.mjs
...
// node_modules/framer-motion/dist/es/animation/js/index.mjs
var types = {
  decay: inertia,
  inertia,
  tween: keyframes,
...
function calculateDuration(generator) {
  ...
  while (!state.done && duration < maxDuration2) {
    duration += timeStep;
...
function createAcceleratedAnimation(value, valueName, { onUpdate, onComplete, ...options }) {
  ...
  if (!canAccelerateAnimation)
    return false;
  let { keyframes: keyframes2, duration = 300, ease: ease2 } = options;
  if (requiresPregeneratedKeyframes(valueName, options)) {
    ...
    let t = 0;
    while (!state.done && t < maxDuration3) {
...
function createInstantAnimation({ keyframes: keyframes2, delay: delayBy, onUpdate, onComplete }) {
  const setValue = () => {
...
function updateMotionValuesFromProps(element, next, prev) {
  ...
  for (const key in next) {
    ...
    if (isMotionValue(nextValue)) {
      ...
      if (true) {
        warnOnce(nextValue.version === "10.3.1", `Attempting to mix Framer Motion versions ${nextValue.version} with 10.3.1 may not work as expected.`);
      }
...
var LayoutGroup = ({ children, id: id5, inherit = true }) => {
  ...
  const context = {
    ...
    updateOrder: (id5, offset, velocity) => {
      if (isReordering.current)
        return;
      const newOrder = checkReorder(order, id5, offset, velocity);
      if (order !== newOrder) {
...
var AnimateSharedLayout = ({ children }) => {
  ...
  return React11.createElement(LayoutGroup, { id: useConstant(() => `asl-${id4++}`) }, children);
};
export {
  AnimatePresence,
  AnimateSharedLayout,
